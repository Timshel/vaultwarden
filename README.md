...
This branch now contains features not added to the SSO [PR](https://github.com/dani-garcia/vaultwarden/pull/3899) since it would slow down the review review.
...
Allow to invite user to existing organization if they are listed in the Access token.
If activated it will check if the token contain a list of potential organization.
If an organization with a matching name (case sensitive) is found it will the start the invitation process for this user.
...
- if mail is activated invite the user to the organization
...
Made a version which additionally allow to run the server without storing the master password (it's still required just not sent to the server).
...
\
When building the docker image it can be overrode by passing the `OIDC_WEB_RELEASE` arg.
\
